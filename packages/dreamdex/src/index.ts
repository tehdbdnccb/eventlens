...
export interface DreamDexAdapter{discoverMarkets():Promise<Market[]>; orderBook(marketId:string):Promise<OrderBook>; execute(input:{marketId:string;side:Side;quantity:number;maxPrice:number;wallet:string}):Promise<Trade>; listTrades?():Trade[];}
